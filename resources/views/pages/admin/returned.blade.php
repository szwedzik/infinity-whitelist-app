                                    ...
                                    <table id="awaiting" class="table table-sm card-table">
                                        ...
                                        @foreach($applications as $application)
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge badge-soft-warning"><i class="fe fe-random"></i> Zwrócone do poprawy</span>
                                                </td>
                                                <td>
                                                    {{ $application->admin }}
                                                </td>
                                                <td id="reason">{{$application->getReason($application->id)}}</td>
                                                <td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        reasons.forEach((reason) => {
            if (reason.textContent.length > 20) {
                const originalText = reason.textContent;
                const truncatedText = originalText.slice(0, 17) + "...";
                reason.textContent = truncatedText;

                reason.addEventListener("mouseenter", () => {
                    reason.setAttribute("title", originalText);
                });

                reason.addEventListener("mouseleave", () => {
                    reason.removeAttribute("title");
                });
            }
        });
    });
